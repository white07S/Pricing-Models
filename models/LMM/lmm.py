...
class RateStructure:
    ...
    def __str__( self ):
        return f"At time {str(self.t)}, the rate structure is {str(self.rates)}"
    ...
    def __interpolate( self, time, timeInd, rateInd ):
        ...
        if ( timeInd == 0 ):
            if ( time == self.__result[ 0 ].t ):
                ...
                raise Exception(f"Trying to access {str(time)} while the earlist time point is {str(self.__result[0].t)}")
            ...
            if ( time == self.__result[ -1 ].t ):
                ...
                raise Exception(f"Trying to access {str(time)} while the oldest time point is {str(self.__result[0].t)}")
...
class vol2( volatility ):
    ...
    def get(self, curTime, criticalTimePoint):
        ...
        return [self.__vol[i] for i in range(len(self.__vol) - k)]
    ...
    def calibrate(self, tarVol, criticalTimePoint, tau):
        ...
        for i in range(len(tarVol)):
            temp = sum(self.__vol[j]**2 * tau[i - 1 - j] for j in range(i))
...
class vol6( volatility ):
    ...
    def __str__(self):
        return f"sigma(t) = [ a( T - t ) + d ] * exp( -b * ( T - t ) ) + c, a = {str(self.a)} b = {str(self.b)} c = {str(self.c)} d = {str(self.d)}"
    ...
    def __toBSVar(self, ctp):
        ...
        temp.extend(self.__integralFun(ctp[i], ctp[i + 1]) + temp[-1] for i in range(len(ctp) - 1))
...
class vol7( volatility ):
    ...
    def __str__(self):
        return f"sigma(t) = phi(i) * [ a( T(i) - t ) + d ] * exp( -b * ( T(i) - t ) ) + c, a = {str(self.a)}, b = {str(self.b)}, c = {str(self.c)}, d = {str(self.d)}" + "\nphi = " + str(self.__phi)
...
class LiborMarketModel( metaclass = ABCMeta ):
    ...
    def __getIVCalc(self, swaptionIV, zcpPrc):
        ...
        start = bisect.bisect_left(self.__ctp, T0)
        end = bisect.bisect_right(self.__ctp, Tn)
